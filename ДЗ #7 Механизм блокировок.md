...
на команду pg_locks в режиме AccessShareLock - вызвана тем, что мы в данный момент просматриваем статистику через `select 
...
db_locks=*# UPDATE test_locks SET descr = ' test 1.1'  WHERE id = 1;
db_locks=*# UPDATE test_locks SET descr = ' test 1.2'  WHERE id = 1;
db_locks=*# UPDATE test_locks SET descr = ' test 1.3'  WHERE id = 1;
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY3MjAzNDMxNSwtMTE0NzYyNDAxNyw0MT
