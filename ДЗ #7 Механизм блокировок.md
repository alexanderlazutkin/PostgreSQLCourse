...
UPDATE test SET value = value + 10 WHERE id = 3; 

 - 3 сессия: set value = 40
UPDATE test SET value = value + 10 WHERE id = 2; 

***Получили сообщение в 3й сессии:***

 - dblocks=*# UPDATE test SET value = value + 10 WHERE id = 2; ERROR:
 - deadlock detected DETAIL:  Process 186 waits for ExclusiveLock on tuple (0,2) of relation 16396 of database 16384; blocked by process 98.
 - Process 97 waits for ShareLock on transaction 747; blocked by process 155.
 - Process 155 waits for ShareLock on transaction 748; blocked by process 186 HINT:  See server log for query details.


***Посмотрим лог сервера***
tail -n 30 /var/log/postgresql/postgresql-14-main.log
2023-03-21 20:31:53.350 MSK [186] postgres@dblocks ERROR:  deadlock detected
2023-03-21 20:31:53.350 MSK [186] postgres@dblocks DETAIL:  Process 186 waits for ExclusiveLock on tuple (0,2) of relation 16396 of database 16384; blocked by process 97.
        Process 97 waits for ShareLock on transaction 747; blocked by process 155.
        Process 155 waits for ShareLock on transaction 748; blocked by process 186.
        Process 186: UPDATE test SET value = value + 10 WHERE id = 2;
        Process 97: UPDATE test SET value = value + 10 WHERE id = 2;
        Process 155: UPDATE test SET value = value + 10 WHERE id = 3;
2023-03-21 20:31:53.350 MSK [186] postgres@dblocks HINT:  See server log for query details.
2023-03-21 20:31:53.350 MSK [186] postgres@dblocks STATEMENT:  UPDATE test SET value = value + 10 WHERE id = 2;
...
----+-------
 ...
 - Обнаружен deadlock. 
 - Процесс 186 ожидает экскл.блокировку на второй строке и заблокирован процессом 97. Процесс 97 ожидает  ShareLock на транзакции 747 и заблокирован процессом 155.        Процесс 155 ожидает ExclusiveLock на транзакции 748, заблокированной процессом 186.
 Где:       
        Process 186: UPDATE test SET value = value + 10 WHERE id = 2;
        Process 97: UPDATE test SET value = value + 10 WHERE id = 2;
        Process 155: UPDATE test SET value = value + 10 WHERE id = 3;

Взаимоблокировка возникла 

#### Могут ли две транзакции, выполняющие единственную команду UPDATE одной и той же таблицы (без where), заблокировать друг друга?

Стандартным оператором UPDATE маловероятно, т.к. блокировка распостранится на всю таблицу.
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjI5NTQ5OTc1LDY2NTczNjU2OCwtMTU0OT
IwNjM0NiwyNzM4MDc2NTIsMTg3MzUzNTUwLC0xMTQ3NjI0MDE3
LDQxOTU5MzM5LDc4MTYwMDU5MywtNjI0MjAwNTc3LDIyMzE2Nz
E2OCwxNDY0MzAzODUxXX0=
-->